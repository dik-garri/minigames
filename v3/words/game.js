/* Слова веры — сборка слов из кубиков на скорость.
   Всё считается на устройстве: страница статическая, бэкенда нет.
   Общий старт даёт отсчёт 3-2-1 на каждом телефоне. */

const WORDS = [
  'МОЛИТВА', 'БИБЛИЯ', 'ЕВАНГЕЛИЕ', 'БЛАГОДАТЬ', 'ПРОЩЕНИЕ',
  'НАДЕЖДА', 'ЛЮБОВЬ', 'СПАСЕНИЕ', 'АМИНЬ', 'РАДОСТЬ',
  'ЦЕРКОВЬ', 'АПОСТОЛ', 'ПСАЛОМ', 'ПРИТЧА', 'ЗАПОВЕДЬ',
  'ПАСТЫРЬ', 'ОБЩЕНИЕ', 'СЛУЖЕНИЕ', 'ДРУЖБА', 'ГИТАРА'
];

const SAVE_KEY = 'faith-words-v3';   // свой ключ, чтобы не подхватить партию v1/v2
...
el.copy.addEventListener('click', async () => {
  ...
  const text = `Слова веры: ${WORDS.length} из ${WORDS.length} за ${time}. Подсказок: ${state.hints} из ${MAX_HINTS}.`;
  try {
